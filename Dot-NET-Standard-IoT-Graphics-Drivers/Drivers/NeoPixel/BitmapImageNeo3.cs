...
namespace Glovebox.Graphics.Drivers.NeoPixel
{
    ...
    public class BitmapImageNeo3 : BitmapImage
    {
        ...
        public BitmapImageNeo3(int width, int height, int panelsPerFrame)
            : base(new byte[(width * height * BytesPerPixel + ResetDelayInBytes) * panelsPerFrame], width, height, width * BytesPerPixel)
        {
        }
